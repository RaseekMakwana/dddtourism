...
class HospitalController extends Controller
{
    ...
    public function index(){
        $data['banner'] = DB::table('banner')->where(['category'=>'Hospitals','state'=> ucfirst($this->segment1)])->get();
        $data['banner_type'] = "banner_with_center_content_position";
        return view('site.facilities.hospital', compact('data'));
    }
    ...
    public function apiGetMasterData(Request $request){
        ...
        try {
            ...
            $slug = Str::slug(strtolower($inputs['state'].'-hospital'));
