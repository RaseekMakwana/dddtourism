...
class TentsController extends Controller
{
    ...
    public function index(){
        ...
        $data['banner'] = DB::table('banner')->where(['category'=>'Tents','state'=> ucfirst($this->segment1)])->get();
        $data['banner_type'] = "banner_with_center_content_position";
        return view('site.facilities.tents', compact('data'));
    }
    ...
    public function apiGetMasterData(Request $request){
        ...
        try {
            ...
            $slug = Str::slug(strtolower($inputs['state'].'-tents'));

            $queryString ='';
            if(!empty($inputs['tent_type'])){
                $queryString .= " AND tent_type='".$inputs['tent_type']."'";
            }

            $per_page_records = 1;
            $facilities = DB::table("tents")->select("*")
    ...
    //     }
    //     try {
    //         $slug = Str::slug(strtolower(session('state').'-tent'));
