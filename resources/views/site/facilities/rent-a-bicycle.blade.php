...
function loadMasterData(page = 1){
    ...
    formData.append("facility_type", 'Rent a Bicycle');
    formData.append("tab_option", $("#hidden_tab_option").val());
    formData.append("page_number",page);

    axios.post(axios.defaults.baseURL+'site/facilities/rent-a-bicycle/get-data', formData).then(function (response) {
        htmlBuilt(response);
...
function htmlBuiltPagination(response){
    ...
    if(meta.total != "0"){
        ...
        $("#facility_total_count_message").text(meta.total+" - Rent A Bicycle in {{ ucfirst($segment1) }}");
    } else {
