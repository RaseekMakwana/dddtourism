        ...
        <div class="row">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0 pb-0">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">Rent a Bike</li>
                </ol>
            </nav>
        </div>
...
function loadMasterData(page = 1){
    ...
    formData.append("facility_type", 'Rent a Bike');
    formData.append("tab_option", $("#hidden_tab_option").val());
    formData.append("page_number",page);

    axios.post(axios.defaults.baseURL+'site/facilities/rent-a-bike/get-data', formData).then(function (response) {
        htmlBuilt(response);
...
function htmlBuiltPagination(response){
    ...
    if(meta.total != "0"){
        ...
        $("#facility_total_count_message").text(meta.total+" - Rent a Bike in {{ ucfirst($segment1) }}");
    } else {
